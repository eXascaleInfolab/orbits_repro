...
{
    public partial class LongCentroidDecompositionAlgorithm : Algorithm
    {
        public List<int> KList = new List<int>(new[] { 3, 2 });

        private static bool _init = false;
        public LongCentroidDecompositionAlgorithm() : base(ref _init)
        { }
        ...
        private Process GetOfflineProcess(DataDescription data, int len, int k, Experiment ex)
        {
            ...
            proc.StartInfo.Arguments = $"-alg cd2 -test {test} -n {data.N} -m {data.M} -k {k} " +
